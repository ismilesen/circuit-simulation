...
# Emitted just before ngspice run_simulation() is called.
...
var _continuous_running: bool = false
...
func _ready() -> void:
	...
	if continuous_button != null:
		continuous_button.text = "Run Live"
...
func _on_run_pressed() -> void:
	# Stop any running continuous simulation before starting a batch run.
	if _continuous_running:
		_sim = _resolve_simulator()
		if _sim != null and _sim.has_method("stop_continuous_transient"):
	...
	# CONNECT_DEFERRED: ngspice emits these from its own OS thread.
	# Without this flag Godot crashes on get_script() when dispatching to GDScript.
	if (not _sim_signal_connected) and _sim.has_signal("simulation_finished"):
		_sim.connect("simulation_finished", Callable(self, "_on_sim_finished"), CONNECT_DEFERRED)
	...
	if (not _continuous_signal_connected) and _sim.has_signal("continuous_transient_started") and _sim.has_signal("continuous_transient_stopped") and _sim.has_signal("continuous_transient_frame"):
		_sim.connect("continuous_transient_started", Callable(self, "_on_continuous_started"), CONNECT_DEFERRED)
		_sim.connect("continuous_transient_stopped", Callable(self, "_on_continuous_stopped"), CONNECT_DEFERRED)
		_sim.connect("continuous_transient_frame", Callable(self, "_on_continuous_frame"), CONNECT_DEFERRED)
	...
	# Resolve PDK root: inspector export > env var > common install locations.
	var effective_pdk_root: String = _resolve_pdk_root()
	if effective_pdk_root != "":
		_log("[color=lightblue]PDK root:[/color] %s" % effective_pdk_root)
	else:
		_log("[color=yellow]Warning: PDK root not found — transistor/std-cell models will be missing.[/color]")
	...
	if not subckt_info.is_empty() and _sim.has_method("load_netlist_string"):
		...
		if not loaded:
			...
			return
		_refresh_status("native: subcircuit loaded — starting simulation…", StatusTone.WARN)
		_run_batch()
		return
...
# Starts or stops the continuous (streaming) transient simulation.
func _on_continuous_pressed() -> void:
	...
	if _continuous_running:
		if _sim != null and _sim.has_method("stop_continuous_transient"):
			_sim.call("stop_continuous_transient")
		# UI update comes from _apply_continuous_stopped_ui via deferred signal.
		return

	# ── Start ─────────────────────────────────────────────────────────────────
	if staged.is_empty():
		_set_error("No staged files. Upload a .spice/.cir netlist first.")
		return
	...
	var entry: Dictionary = staged[int(idx[0])]
	if not _is_netlist_entry(entry):
		_set_error("Selected file is not a netlist. Choose .spice, .cir, .net, or .txt.")
		return

	if OS.has_feature("web"):
		_set_error("Web build: ngspice continuous simulation is not supported.")
		return
	...
	# Auto-load the paired schematic so the 3D scene is ready.
	var sch_pair: Dictionary = _find_matching_sch(entry)
	if not sch_pair.is_empty():
		_log("[color=lightblue]Pairing schematic:[/color] %s" % str(sch_pair["display"]))
		schematic_requested.emit(str(sch_pair["user_path"]))
	...
	if effective_pdk_root != "":
		...
		_log("[color=yellow]Warning: PDK root not found — models may be missing.[/color]")

	if not bool(_sim.call("initialize_ngspice")):
		_set_error("initialize_ngspice() returned false.")
		return

	# Wire up continuous signals once.
	if not _continuous_signal_connected and _sim.has_signal("continuous_transient_started") \
			and _sim.has_signal("continuous_transient_stopped") \
			and _sim.has_signal("continuous_transient_frame"):
		_sim.connect("continuous_transient_started",  Callable(self, "_on_continuous_started"),  CONNECT_DEFERRED)
		_sim.connect("continuous_transient_stopped",  Callable(self, "_on_continuous_stopped"),  CONNECT_DEFERRED)
		_sim.connect("continuous_transient_frame",    Callable(self, "_on_continuous_frame"),    CONNECT_DEFERRED)
		_continuous_signal_connected = true

	# Load the netlist — use a long-window wrapper for subcircuits.
	var subckt_info: Dictionary = _parse_subcircuit_info(os_path)
	if not subckt_info.is_empty() and _sim.has_method("load_netlist_string"):
		var wrapper: String = _build_continuous_wrapper(
			os_path, str(subckt_info["name"]), subckt_info["ports"] as Array, effective_pdk_root
		)
		_log("[color=lightblue]Continuous subcircuit wrapper generated for:[/color] %s" % str(subckt_info["name"]))
		if not bool(_sim.call("load_netlist_string", wrapper)):
			_set_error("Failed to load wrapper for continuous simulation.")
			return
	elif _sim.has_method("load_netlist"):
		var result: Variant = _sim.call("load_netlist", os_path, effective_pdk_root)
		if typeof(result) != TYPE_DICTIONARY or (result as Dictionary).is_empty():
			_set_error("load_netlist() returned no data.")
			return

	# Configure RAM sample buffer so the visualizer can stream data.
	...
	# Use 1 ns step; 1 ms total window (long enough for multi-cycle counter, short enough to load fast).
	var step: float    = maxf(continuous_step, 1e-10)
	var max_time: float = 1e-3
	simulation_started.emit()
	_refresh_status("native: starting continuous simulation…", StatusTone.WARN)
	var ok: bool = bool(_sim.call("start_continuous_transient", step, max_time))
	if not ok:
		_set_error("start_continuous_transient() returned false.")


# Generates a simulation wrapper with a long transient window for continuous streaming.
# Identical to the batch wrapper except the .tran card uses a 1 ms window instead of 1200 ns.
func _build_continuous_wrapper(os_path: String, subckt_name: String, ports: Array, effective_pdk_root: String) -> String:
	var batch: String = _build_sim_wrapper(os_path, subckt_name, ports, effective_pdk_root)
	# Replace the short batch .tran card with a much longer one for continuous mode.
	var lines: PackedStringArray = batch.split("\n")
	var out: Array[String] = []
	for line: String in lines:
		if line.strip_edges().to_lower().begins_with(".tran "):
			out.append(".tran 1n 1m")
		else:
			out.append(line)
	return "\n".join(out)


# Runs a batch (one-shot) simulation. simulation_finished fires when ngspice finishes.
func _run_batch() -> void:
	if _sim == null:
		return
	if not _sim.has_method("run_simulation"):
		_set_error("CircuitSimulator build does not expose run_simulation().")
		return
	simulation_started.emit()
	_refresh_status("native: simulation running…", StatusTone.WARN)
	var ok: bool = bool(_sim.call("run_simulation"))
	if not ok:
		_set_error("run_simulation() returned false.")

# Updates UI when batch simulation completes.
func _on_sim_finished() -> void:
	_refresh_status("native: simulation complete", StatusTone.OK)
...
func _apply_continuous_started_ui() -> void:
	...
	if continuous_button != null:
		continuous_button.text = "Stop Live"
	if run_button != null:
		run_button.disabled = true
	_refresh_status("native: continuous simulation running", StatusTone.OK)
...
func _apply_continuous_stopped_ui() -> void:
	...
	_refresh_status("native: continuous simulation stopped", StatusTone.WARN)
...
func _apply_continuous_frame_ui(frame: Dictionary) -> void:
	...
	var sim_ns: float = sim_time * 1e9
	_refresh_status(
		"continuous  frame %d  |  samples: %d  |  t: %.1f ns" % [_continuous_frame_count, sample_count, sim_ns],
		StatusTone.OK
	)
...
# Clears staged files and output logs.
...
# Returns {name, ports} if the file is a bare subcircuit definition with no analysis
# commands anywhere in the file. Returns {} for complete decks or non-spice files.
# Handles analysis commands both outside (.tran) and inside .control blocks (tran).
func _parse_subcircuit_info(path: String) -> Dictionary:
	...
	for raw_line: String in text.split("\n"):
		...
		if line.begins_with(".endc"):
			...
			continue
		if in_control:
			# Inside .control: analysis commands have no leading dot (e.g. "tran 1n 100n")
			if line.begins_with("tran ") or line.begins_with("ac ") or line.begins_with("dc ") or line == "op" or line.begins_with("op "):
				has_analysis = true
			continue
		# Outside .control
...
# Resolves the PDK root path: inspector export > env var > common install locations.
func _resolve_pdk_root() -> String:
	if pdk_root != "":
		return pdk_root
	var env := OS.get_environment("PDK_ROOT")
	if env != "":
		return env
	# Common install paths on Windows and Linux.
	for candidate: String in ["C:/pdk", "C:/PDK", "/usr/share/pdk", "/usr/local/share/pdk"]:
		if DirAccess.dir_exists_absolute(candidate + "/sky130A"):
			return candidate
	return ""


# Scans a subcircuit SPICE file for internal nets that look like clock signals
# (CLK, CK, CLOCK) and are NOT in the declared port list.
func _find_internal_clocks(os_path: String, ports: Array) -> Array[String]:
	var text := FileAccess.get_file_as_string(os_path)
	var port_upper: Dictionary = {}
	for p: String in ports:
		port_upper[p.to_upper()] = true

	var found: Dictionary = {}
	var in_subckt := false
	for raw_line: String in text.split("\n"):
		var line := raw_line.strip_edges()
		var ll := line.to_lower()
		if ll.begins_with(".subckt"):
			in_subckt = true
			continue
		if ll.begins_with(".ends"):
			in_subckt = false
			continue
		if not in_subckt or not ll.begins_with("x"):
			continue
		# Instance line: x<name> <net0> <net1> ... <subckt_model>
		var parts := line.split(" ", false)
		for i: int in range(1, parts.size() - 1):
			var net := parts[i].to_upper()
			if port_upper.has(net):
				continue
			if "CLK" in net or net == "CK" or "CLOCK" in net:
				found[parts[i].to_lower()] = true

	var result: Array[String] = []
	for k: String in found.keys():
		result.append(k)
	return result


## Returns all unique internal node names inside the first .subckt block —
## i.e. net tokens that appear in component connection lines but are NOT
## in the declared port list.  Handles multi-line continuation (+) correctly.
func _extract_internal_nodes(os_path: String, ports: Array) -> Array[String]:
	var text := FileAccess.get_file_as_string(os_path)
	if text.is_empty():
		return []

	var port_set: Dictionary = {}
	for p: String in ports:
		port_set[p.to_upper()] = true
	port_set["0"] = true  # SPICE ground reference

	var internal: Dictionary = {}
	var in_subckt := false
	var acc := ""  # accumulates continuation lines
	...
	for raw: String in text.split("\n"):
		...
		if line.begins_with("+"):
			...
			continue
		if acc != "":
			_scan_line_nodes(acc, port_set, in_subckt, internal)
		acc = line
		var ll := line.to_lower()
		if ll.begins_with(".subckt"):
			in_subckt = true
			acc = ""
		elif ll.begins_with(".ends"):
			in_subckt = false
			acc = ""
	if acc != "":
		_scan_line_nodes(acc, port_set, in_subckt, internal)
...
## Scans one SPICE component line for net tokens and adds internals to `out`.
## For X-instances the last non-param token is the subcircuit model name — skipped.
func _scan_line_nodes(line: String, port_set: Dictionary, in_subckt: bool, out: Dictionary) -> void:
	if not in_subckt:
		return
	...
	var param_start := parts.size()
	for i in range(1, parts.size()):
		if "=" in parts[i]:
			param_start = i
			break
	# Tokens [1 .. net_end] are net connections.
	# For X-prefixed instances, skip the last pre-param token (= model/subckt name).
	var net_end := param_start - 1
	if parts[0].to_upper().begins_with("X") and net_end >= 2:
		net_end -= 1
	for i in range(1, net_end + 1):
		var tok := parts[i]
		if tok.is_empty() or tok.is_valid_float() or tok.is_valid_int():
			continue
		if port_set.has(tok.to_upper()):
			continue
		out[tok.to_lower()] = true
...
func _build_sim_wrapper(os_path: String, subckt_name: String, ports: Array, effective_pdk_root: String) -> String:
	var lines: Array[String] = []
	lines.append("* auto-simulation wrapper for " + subckt_name)
	var abs_path: String = os_path.replace("\\", "/")

	if effective_pdk_root != "":
		var pdk_fwd: String = effective_pdk_root.replace("\\", "/")
		# Transistor primitive models — try known sky130 lib locations in order.
		var lib_candidates: Array[String] = [
			pdk_fwd + "/sky130A/libs.tech/ngspice/sky130.lib.spice",
			pdk_fwd + "/sky130A/libs.tech/combined/sky130.lib.spice",
		]
		for candidate: String in lib_candidates:
			if FileAccess.file_exists(candidate):
				lines.append(".lib \"" + candidate + "\" tt")
				break
		# Standard cell subcircuit definitions (dfxtp, ha, inv, etc.)
		var sc_spice: String = pdk_fwd + "/sky130A/libs.ref/sky130_fd_sc_hd/spice/sky130_fd_sc_hd.spice"
		var subckt_text: String = FileAccess.get_file_as_string(os_path)
		# Only include the sc_hd library when the file references OTHER sc_hd cells as
		# subcircuits (not its own definition). If subckt_name is itself an sc_hd cell,
		# including the library creates a duplicate definition that aborts ngspice parsing.
		if FileAccess.file_exists(sc_spice) and "sky130_fd_sc_hd__" in subckt_text and not subckt_name.to_lower().begins_with("sky130_fd_sc_hd__"):
			lines.append(".include \"" + sc_spice + "\"")
	...
	var output_hints: Array = ["Q", "QN", "Z", "Y", "X", "OUT"]
	...
	# Drive any internal clock nets (not in the port list) via hierarchical node names.
	var internal_clks := _find_internal_clocks(os_path, ports)
	for clk: String in internal_clks:
		lines.append("VCLK_%s x_dut.%s 0 PULSE(0 1.8 0 2n 2n 100n 200n)" % [clk, clk])
