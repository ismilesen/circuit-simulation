...
namespace {
#if defined(CIRCUIT_SIM_DISABLE_NGSPICE)
#if defined(__clang__) || defined(__GNUC__)
#pragma message("CircuitSimulator: ngspice runtime is disabled (CIRCUIT_SIM_DISABLE_NGSPICE).")
#endif
constexpr bool kNgspiceRuntimeSupported = false;
#else
constexpr bool kNgspiceRuntimeSupported = true;
#endif

String ngspice_runtime_unavailable_reason() {
    return String("ngspice runtime is disabled for this build (CIRCUIT_SIM_DISABLE_NGSPICE).");
}
...
void CircuitSimulator::_bind_methods() {
    ...
    ClassDB::bind_method(D_METHOD("is_ngspice_runtime_supported"), &CircuitSimulator::is_ngspice_runtime_supported);
    ClassDB::bind_method(D_METHOD("get_ngspice_runtime_message"), &CircuitSimulator::get_ngspice_runtime_message);
...
bool CircuitSimulator::is_ngspice_runtime_supported() const {
    return kNgspiceRuntimeSupported;
}

String CircuitSimulator::get_ngspice_runtime_message() const {
    if (kNgspiceRuntimeSupported) {
        return String("ngspice runtime is available on this build.");
    }
    return ngspice_runtime_unavailable_reason();
}

// Dynamically loads the ngspice shared library and required symbols.
bool CircuitSimulator::load_ngspice_library() {
#if defined(CIRCUIT_SIM_DISABLE_NGSPICE)
    UtilityFunctions::printerr(ngspice_runtime_unavailable_reason());
    return false;
#elif defined(_WIN32)
...
#ifdef __EMSCRIPTEN__
        // Browser dlopen cannot synchronously fetch external files; keep probing
        // minimal to avoid duplicate-load assertions while a prior async load is pending.
        if (last_error.find("Attempt to load") != -1 ||
            last_error.find("synchronous loading of external files is not available") != -1) {
            break;
        }
#endif
    }
    ...
    if (!ngspice_handle) {
        ...
        UtilityFunctions::printerr(
            "Web ngspice side-module missing. Expected file: ngspice/libngspice.so "
            "(relative to exported index.html).");
#endif
        return false;
    }
...
Dictionary CircuitSimulator::load_netlist(const String &netlist_path, const String &pdk_root) {
    ...
    if (!kNgspiceRuntimeSupported) {
        const String reason = ngspice_runtime_unavailable_reason();
        UtilityFunctions::printerr(reason);
        result["error"] = reason;
        return result;
    }
...
bool CircuitSimulator::start_continuous_transient(double step, double window, int64_t sleep_ms) {
    ...
    // Web exports are currently single-threaded. Running this path synchronously
    // avoids std::thread/pthread aborts in nothreads builds.
    bool ok = true;
    {
...
void CircuitSimulator::stop_continuous_thread() {
    ...
    if (initialized && ng_Command && ng_Running && ng_Running()) {
